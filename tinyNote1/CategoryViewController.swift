...
class CategoryViewController: UIViewController, NSFetchedResultsControllerDelegate, UITableViewDataSource, UITableViewDelegate, WCSessionDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let arequest = NSFetchRequest(entityName: "CategoryList")
        let atitleSort = NSSortDescriptor(key: "categoryName", ascending: true)
        arequest.sortDescriptors = [atitleSort]
        arequest.resultType = NSFetchRequestResultType.DictionaryResultType
        arequest.returnsDistinctResults = true
        arequest.propertiesToFetch = ["categoryName"]
        arequest.returnsObjectsAsFaults = false
        do {
        let results:NSArray = try context.executeFetchRequest(arequest)
            print(results[0])
